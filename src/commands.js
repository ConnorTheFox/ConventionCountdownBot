...
async function menu(msg) {
	returns.generateLog(msg.chat.title || msg.chat.first_name, 'start', 'command')
	bot.sendMessage(msg.chat.id, conInfo.start, { parse_mode: 'markdown', reply_markup: generateButtons('start', await db.find({ chatId: msg.chat.id }, 'users')) })
}
...
function generateButtons(type, buttonChoice) { //This creates buttons for the menu, add to the switch for custom menu
	switch (type) {
		...
		case 'commands':
			let tempArr = []
			for (let x in conInfo.help) { //This is for commands list
				tempArr.push([{ text: x, callback_data: conInfo.help[x] }])
			}
...
async function answerKeyboard(item) { //This answers the request for keyboard, called on response
	...
	if (command === 'start') {
		bot.editMessageText(conInfo.start, { chat_id: chatId, message_id: messageId, reply_markup: generateButtons('start', await db.find({ chatId: chatId }, 'users')) })
	}
	...
	async function sendCountdownReply(countdownType) { //Logic to handle both countdown options
		if (type != 'supergroup' || (type === 'supergroup' && await checkForAdmin())) {
			...
			if (status) {
				bot.editMessageText(conInfo.countdown[countdownType], { chat_id: chatId, message_id: messageId, reply_markup: generateButtons('command') })
				if (countdownType === 'start') {
					conInfo.owner.forEach(ownerId => {
						bot.sendMessage(ownerId, (item.message.chat.title || item.message.chat.first_name) + ' Subscribed!')
					})
				}
			} else {
				bot.editMessageText(conInfo.countdown.else[countdownType], { chat_id: chatId, message_id: messageId, reply_markup: generateButtons('command') })
			}
		} else {
	...
	if (command === 'info') {
		bot.editMessageText(conInfo.info, { chat_id: chatId, message_id: messageId, reply_markup: generateButtons('command') })
	}

	if (command === 'daysleft') {
		let day = days.until()
		bot.editMessageText(`${day} days until ${conInfo.con}`, { chat_id: chatId, message_id: messageId, reply_markup: generateButtons('command') })
	}
}
...
async function addedToGroup(msg) {
	...
	if (msg.new_chat_participant.id === botId) {
		bot.sendMessage(msg.chat.id, conInfo.start, { parse_mode: 'markdown', reply_markup: generateButtons('start') })
	}
}
